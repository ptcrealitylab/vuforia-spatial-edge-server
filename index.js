...
function onChildCrash(code, signal) {
    if (code > 0 || signal) {
        console.info('server.js has exited expectedly');
        return;
    }
...
function onElectronChildMessage(message) {
    if (message === 'restart') {
        console.log('message from child:', message);
        app.relaunch();
        app.exit(0);
...
if (app) {
    ...
    process.send = onElectronChildMessage; // hack
