...
//    console.log("writeIOToServer obj: "+objectName + "  name: "+nodeName+ "  value: "+value+ "  mode: "+mode);
    ...
    if (objects.hasOwnProperty(objKey2)) {
        if (objects[objKey2].nodes.hasOwnProperty(nodeName)) {
            if (objects[objKey2].nodes[nodeName].blocks.hasOwnProperty(defaultBlock)) {
           ...
           var objectID = HybridObjectsUtilities.getObjectIdFromTarget(objectName, dirnameO);
            ...
            if (!_.isUndefined(objectID) && !_.isNull(objectID) && objectID.length > 13) {
                for (var key in objects[objectID].nodes) {
                    if (!hardwareInterfaces[type].hybridObjects[objectName].nodes.hasOwnProperty(objects[objectID].objectValues[key].name)) {
