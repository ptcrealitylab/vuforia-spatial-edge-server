...
router.post('/query', async function(req, res) {
    ...
    // let myUserMessage = '';
    // if (myUser) {
    //     myUserMessage = `The user interacting with you is named ${myUser.name}. If the user asks about "here", "this", or other indirect spatial references, assume they mean their spatial cursor position, which is currently ${myUser.spatialCursorPosition}. Try to infer what the user means based on this information.`;
    // }
...
- 'applicationId' and 'applicationType' describe the unique identifier and type of the application.
- 'worldPosition' and 'worldMatrix' provide the spatial orientation and position in the scene.
Use this information to understand the placement and configuration of applications relative to their parent objects in the scene, facilitating accurate responses to user queries about spatial interactions within the digital twin environment.\n${JSON.stringify(simplifiedDataModel)}`;
